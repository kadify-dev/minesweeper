...
class Server:
    ...
    def process_game(self):
        for _ in range(settings.FIELD_SIZE * settings.FIELD_SIZE):
            for player in self.players:
    ...
    def send_end_game_data(self, player: Player):
        message = "Вы проиграли." if player.is_lose else "Вы выйграли!"
        send(player.client_socket, {"message": message})
        data = receive(player.client_socket)
    ...
    def run(self):
        ...
        for player in self.players:
            self.send_end_game_data(player)
